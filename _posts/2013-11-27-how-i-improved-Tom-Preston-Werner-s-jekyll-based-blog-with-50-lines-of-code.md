...
You may have heard about [lunr](http://lunrjs.com) and its [jekyll integration](https://github.com/slashdotdash/jekyll-lunr-js-search), you can find a demo [here](http://10consulting.com/search). It's pretty awesome but lacks of critical features for now (most critical: stemming doesn't replace typo-tolerance, matching terms are not highlighted [issue 25](https://github.com/olivernn/lunr.js/issues/25)).

It's now about 3 months that I've joined [Algolia](http://www.algolia.com) and I cannot let a day go by without trying to plug our search-engine somewhere. Wow, my personal blog it totally "somewhere". Let's give it a try; Algolia is _NOT_ a site-search engine; but it could be the perfect match to easily jump on old posts.

STOP! You said you don't want to pay anything; Algolia is a [paying service](http://www.algolia.com/pricing). You're right, just [send me an email](mailto:sylvain+from+algolia+dot+com) and I'll convert your 14-day free trial to an unlimited _custom_ free plan.

First, I need to patch my dependencies (I use a `Gemfile` and you should too :p):

{% gist 7664465 Gemfile %}

Let's make it the proper way and add all configuration stuffs in my `_config.yml`.
